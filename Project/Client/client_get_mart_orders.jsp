...
if(rs.next())
{
	...
	do
	{
		if(rs.getString("order_from").equals("huas_guest"))
		{
			rs1=cnn.createStatement().executeQuery("select guest_name,concat(guest_address,\", \",guest_city,\" - \",guest_pincode) as guest_add,guest_contact from MART_GUEST_DETAILS where order_id='"+rs.getString("order_id")+"'");
			if(rs1.next())
			{
				%>
				<tr>
					...
					<td style="text-align:left;">
						<b><%=rs1.getString("guest_name")%></b><br>
						<i><%=rs1.getString("guest_add")%></i><br>
						Contact: <i><%=rs1.getString("guest_contact")%></i>
					</td>
