...
String mart_db="";
String mart_name="";
float mart_threshold_purchase=0;
ResultSet rs=DriverManager.getConnection("jdbc:mysql://localhost/HUAS_MARTS","hucky","13071992").createStatement().executeQuery("select mart_userid,mart_threshold_purchase from MART_REGISTER where mart_id='"+mart_id+"'");
if(rs.next())
{
	...
	if(rs1.next())
	{
		...
		mart_name=rs1.getString("firm_name");
                mart_threshold_purchase=Float.parseFloat(rs.getString("mart_threshold_purchase"));
	}
	rs1.close();
}
rs.close();
...
var purchase_threshold=<%=mart_threshold_purchase%>;
...
function pushItem(itemid,cost)
{
	...
	{
		//alert(qty);
                index=selected_items.length;
		selected_items.splice(index,1,itemid);
		selected_items_qty.splice(index,1,qty);
		selected_items_cost.splice(index,1,cost);
                purchase_amount+=parseInt(qty)*parseFloat(cost);
...
{
	qty=selected_items_qty.splice(selected_items.indexOf(itemid),1);
	cost=selected_items_cost.splice(selected_items.indexOf(itemid),1);
        selected_items.splice(selected_items.indexOf(itemid),1);
        purchase_amount-=parseInt(qty)*parseFloat(cost);
	$("#item_"+itemid).attr("onclick","pushItem('"+itemid+"','"+cost+"')");
...
{
    if(purchase_amount>=purchase_threshold)
    {
        return true;
    }
    else
    {
        if(confirm("Your net purchase amount is Rs."+purchase_amount+" which is less than amount Rs."+purchase_threshold+" required for free delivery. \nProceed to order anyway?"))
        {
            return true;
        }
        else
...
function placeOrder(ordered_items_prices)
{
	...
	{
		var guest_details_qs=$("#guest_details_form").serialize();
		//alert(guest_details_qs);
	if(confirm("Submit Order?"))
	{
		$("#extra_inner_box").html("<center><br><br><img src=\"images/loading.gif\"><br><br><h3>Please Wait...</h3></center>");
		$.post("E-Mart/mart_submit_order.jsp","ordered_items="+selected_items+"&ordered_qty="+selected_items_qty+"&ordered_items_prices="+ordered_items_prices+"&"+guest_details_qs,
