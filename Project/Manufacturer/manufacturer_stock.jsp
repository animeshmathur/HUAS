...
{
	$("#add_stock_form").css("visibility","visible");
}

function closeAddStockForm()
{
	$("#add_stock_form").css("visibility","hidden");
}

function checkAddStockForm()
{
	pid=document.getElementById('add_stock_product_id').value;
	quantity=document.getElementById('add_stock_quantity').value;
	if(pid=="-1"||quantity=="")
	{
		alert("Please fill important fields properly!");
		return false;
	}
}

function editStockEntries(pid,pname)
{
	$("#stock_list").fadeOut(
	function()
	{
		$("#product_stock_entries").html("<span style=\"color:#c40e0e;font-size:20px;\">Loading...</span>");
		$("#product_stock_entries").load("manufacturer_stock_get_entries.jsp","pid="+pid+"&pname="+pname);
	}
	);
} 
...
if(rs.next())
{
	...
	<div id="product_stock_entries"></div>
	<%
}
else
{
	%>
	<span style="font-size:20px;color:#006600;">* No Products Available *</span>
	<%
}
%>
</center>
